...
class DeformNPC:
    def __init__(self, r, ringAngles, z, symmet = 8, elliptical = False, 
        ...
        Ks = [] # circulant matrix of spring constants # TODO: Way to lower and increase spring constants of entire model?
        ...
        for i in range(self.nRings):  
            initcoord = self.initialcoords(r_corrected[i], self.z[i], ringAngle = self.ringAngles_corrected[i]) 
        ...
        self.fmag = self.forcesMultivariateNorm(self.initcoords, r_corrected, mag, sigma = sigma) # generate random forces
        self.zoffset = self.forcesMultivariateNorm(self.initcoords, r_corrected, zmag, sigma = sigma, addim = 2)
        ...
        for i in range(self.nRings):
            anchor[2] =  self.z[i]
            self.solution.append(solve_ivp(self.npc, tspan, y0s[i], t_eval = self.teval, 
                                            rtol = 1e-1, atol = 1e-2, 
                                            args=(self.initcoords[i], 
                                            r_corrected[i], Lrests[i], 
                                        Ks[i], kr, self.fmag[i], damp, nConnect, anchor)))   
            
            self.fcoord[i] = self.initialcoords(r_corrected[i], self.z[i], self.ringAngles_corrected[i], self.fmag[i])
    ...
    def npc(self, t, y, initcoords, r, Lrest, K, kr, fmag, damp, nConnect, anchor):
        ...
        return self.npcIntAnc(self.symmet, y, anchor, r, fmag, nConnect, K, Lrest, kr, damp)
    ...
    def npcIntAnc(symmet, y, anchor, r, fmag, nConnect, K, Lrest, kr, damp):
        ...
        allaccarray = np.zeros((symmet, 3)) # array for accelerations of node 0 - symmet-1
 
        for i in range(symmet): # i indicates the reference node        
            accarray = np.array([0., 0., 0.]) # initiate acceleration array for each node i 
            ...
            accarray += kr * (x[i] - anchor)/np.linalg.norm(x[i] - anchor) * (r - np.linalg.norm(x[i] - anchor)) #anchor
    ...
    @staticmethod 
    @njit   
    def adjustRadiusStatic(r8, symmet): #TODO: Why is this part of the deformation code; could be elsewhere
        ...
        """
        alpha = 2*np.pi/symmet # Angle at the tip of triangular slice (pointing to center of NPC)
    ...
    def initialcoordsStatic(r, z, symmet, ringAngle = 0, forces = 0):
        ...
        forces = forces * np.ones(symmet) # forces is 0 or np.array with len(self.symmet)
        rotAngle = np.linspace(0, (symmet-1) * 2*np.pi / symmet, symmet) # angles of nodes around NPC
    ...
    def forcesMultivariateNorm(self, initcoords, r, mag, sigma, addim = 0):
        ...
        *coordring: Initial coordinates of nodes for an arbitrary number of rings. 
        ...
        allcoords3D = np.asarray(initcoords).reshape(nodesTotal, 3) # separated by NPC rings
        
        # compute offset for ellipticity if minor/major axis isn't None or 1, and only in 2D 
        computeFe = True if self.elliptical and self.elliptical != 1 and addim == 0 else False
    ...
    def ellipse(self, allcoords, elliptical):
        ...
        sign = np.sign(difference[:, 0] * allcoords[:, 0] + difference[:, 1] * allcoords[:, 1])
...
def MultipleNPC(nup, term, model, n = 1, rel = False, rvar = None, thetavar = None, 
    ...
    NucSideBool = np.tile(np.repeat(isnuc , symmet), n) #node on nuclear side or not (all NPCs)
    ...
    rexp = DeformNPC(rexp, ringAnglesExp, zexp, symmet = symmet, elliptical = elliptical,
                mag = 0, zmag = 0, sigmamult = sigmamult, nConnect = nConnect, seed = seeds[i]).r


    ringAngles_corrected = deformNPC_temp.ringAngles_corrected # for NPCs that aren't 8-fold symmetric

    multipleNPCs = {"NPCs" : NPCs, "nupIndex" : nup_is, "sigmamult" : sigmamult, "zexp" : zexp, "fcoords" : fcoords,
     "rexp" : rexp, "ringAnglesExp" : ringAnglesExp, 
     "ringAngles_corrected" : ringAngles_corrected, "elliptical" : ellipticals, 
     "fmags" : fmags, "nConnect" : nConnect, 
     "thetaold" :thetaold, "newthetas" : newthetas, "theta_offset" : thetaoffset, 
     "zoffsets" : zoffsets, "nuclear_side_boolean" : NucSideBool, "isnuc": isnuc, 
     "ringmemall": ringMembAll, "ringmember": ringMember, "z_i_all": z_i_all}
...
def Change_radius(r, rnew = False, rsigma = False, seed = None):
    ...
    if str(rnew) == "0" or str(rnew) == "0.0": sys.exit("Radius can't be 0")
...
def Sol3D(solution): #TODO: 3D
    ...
    symmet = int(len(solution.y)/6) #/6 because of 3 dim times both velocity and position 
...
def MultipleNPCs_coord(NPCs, zoffsets, symmet, NucSideBool, nupIndex, tiltnucv, tiltcytv, shiftNuc, shiftCyt, seed = None, frame = -1): #TODO: 3D
    "Input is OdeResults for each NPC in a list. Output is just the final coordinates of each NPC"
    
    nNPCs = len(NPCs)
    nRings = len(NPCs[0]) # TODO: 
    NPCscoord = np.zeros((nNPCs*nRings*symmet, 5)) # number of nodes, dimensions + label #TODO: more general
    # 0 is first frame, -1 is last frame
    i = 0
    
    for NPC in range(nNPCs):        
        for ring in range(nRings):
            NPCscoord[i*symmet : (i+1)*symmet] = np.c_[(Pos3D(NPCs[NPC][ring])[frame, : ],
                np.repeat(nupIndex[NPC][ring], symmet), np.repeat(NPC, symmet))]
            NPCscoord[i*symmet : (i+1)*symmet, 2] +=  zoffsets[NPC][ring] # offset in z
...
def tilt(NPC, NPCscoord, NucSideBool, tiltnucv, tiltcytv):
    ...
    meanzN = np.mean(NPCscoord[Nuc_of_NPCBool, 2]) # mean Z position of NPC nucleoplasmic side
    ...
    NPCscoord[Nuc_of_NPCBool, :3] = Analyse_deformed.rodrigues_rot(NPCscoordsN, baseZ, tiltnucv[NPC]) 
    NPCscoord[Cyt_of_NPCBool, :3] = Analyse_deformed.rodrigues_rot(NPCscoordsC, baseZ, tiltcytv[NPC])
...
def tiltvectors(kappa, nNPCs, seed): #TODO: cite 
    ...
    """
    if (kappa == None): return None, None 
    rng = np.random.default_rng(seed)
    ...
    def rand_von_mises_fisher(mu, kappa, N): 
        ...
        samples[:,1:] = np.matlib.repmat(np.sqrt(1 - t**2), 1, p-1) * xi #could probably be simplified to = np.sqrt(1 - t**2) * xi
