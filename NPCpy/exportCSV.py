...
class MakeCSV:
    def __init__(self, var, NPCs, printNPC, nameDict, name, data_dir): 
        ...
        f.write("Nup index per ring: (corresponding to channel): " + str(NPCs["nupIndex"][0]) + "\n")
        f.write( "Ring membership to subcomplex: " + str(list(NPCs["ringmember"])) + "\n")
        f.write("expected radii per ring: " + str(NPCs["rexp"]) + "\n")
        f.write("expected z per ring: " + str(NPCs["zexp"]) + "\n") 
        f.write("expected ring angles per ring (rad): " + str(NPCs["ringAnglesExp"]) + "\n")
        f.write("twist angle input (rad): " + str(NPCs["thetaold"]) + "\n")
        f.write("Nearest rotational unit, NR - CR (degrees): " + str(math.degrees(NPCs["theta_offset"])) + "\n")
        f.write("\n")
...
class featuresCSV:
    def __init__(self, nameDict, NPCs, var, name, circle_allrings, ellipse_allrings, data_dir):
        ...
        with open(self.csvpath, 'w', newline='') as csvfile: 
            ...
            featuresAll, centreAll, tiltAll = Analyse_deformed.meanfeaturesC(NPCs, var, circle_allrings)
            featuresElAll, centreElAll, tiltElAll = Analyse_deformed.meanfeaturesE(NPCs, var, ellipse_allrings, el_name)
...
def col_features(NPCs, circle_CRNR, ellipse_CRNR):
    ...
    memberof = list(NPCs["ringmember"][np.sort(idx)]) # list of subcomplexes, e.g. ['CR', 'NR']
    dim = 3 # dimensions
    ...
    for NPC in range(n):               
        ...
        entriesel = [ellipse_CRNR[sub][NPC][featEl[i]] for i in range(nfEl) for sub in memberof] # entries alterate between subcomplexes
        featureselAll[NPC] = np.array(entriesel[:len(memberof)*7]) # 'el_minor', 'el_major', 'el_q', 'el_rot', 'el_ssum', 'el_ssumXY', 'el_ssumZ'
        featuresel3DAll[NPC] = np.array(entriesel[len(memberof)*7:]).flatten() # 'Ce', 'normal2', alternating by subcomplex
...
class featuresCSV_subcomplex:
    def __init__(self, NPCs, nameDict, circle_CRNR, ellipse_CRNR, name, data_dir):
        ...
        with open(self.csvpath, 'w', newline='') as csvfile: # TODO: change directory 
            ...
            entriesAll, entriescenAll, entriestiltAll, featureselAll, featuresel3DAll = col_features(NPCs, circle_CRNR, ellipse_CRNR)
