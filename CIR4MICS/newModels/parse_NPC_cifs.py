...
nupnames = ["RANBP2", "NUP210", "ALADIN", "NUP93", "NUP188", "NUP205", "NUP155",
            "NDL1", "NUP35", "NUP54", "NUP58NUP45", "NUP62", "NUP133", "NUP107",
            "NUP98NUP96v1", "SEC13", "SEH1", "NUP85", "NUP43", "NUP160", "NUP37",
            "ELYS", "NUP98NUP96v2", "NUP214", "NUP88"]
...
atomID = list(block.find_loop('_atom_site.auth_atom_id')) # type of atom, we'll later filter for C alpha
seqID = list(block.find_loop('_atom_site.auth_seq_id')) # index of amino acid of interest
...
for value in unique_id:
    indicesN.append(allvalues.asymID[allvalues.asymID == value].index[0]) # first aa [0] is most N-terminal
for value in unique_id:
    indicesC.append(allvalues.asymID[allvalues.asymID == value].index[-1]) # last aa [-1] is most C-terminal
...
nupsall = np.array([str(nup[0]) for nup in list(unique_id)]) # nup IDs, repeated by their frequency

nupsdict = {nups[i]: nupnames[i] for i in range(len(nups))} # dictionary, keys are nups IDs, entries indicate their names
count = Counter(nupsall) # dictionary, keys are nup IDs, entries indicate their frequency

if output: # print Nup coordinates into a .txt file in a format that can be copy/pasted to Nups_Info.py
    with open(outputName, 'w') as f:
        ...
        for i in unique_id: # for each type of Nup 
            ...
            if mode == "N":
                ...
                f.write("\t\t" + authnameN + " = np.array([" + entry + "])\n") # coordinates
            ...
            #### C-terminus
            if mode == "C":
                for i in idlist: # repeat what's been written for N-terminus with C-terminus
                    ...
                    f.write("\t\t" + authnameC + " = np.array([" + entry + "])\n") # coordinates
