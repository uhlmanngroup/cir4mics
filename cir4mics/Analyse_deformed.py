...
def Ellipses(NPCscoords, membership = None):
    ...
    def roundfeatures(ellipsefeatures, z = None, ring = None):
        ...
        ellipsefeatures["el_ssum" + suffix] = round(ellipsefeatures["el_ssum" + suffix], ndigits = 1)
        ellipsefeatures["el_ssumXY" + suffix] = round(ellipsefeatures["el_ssumXY" + suffix], ndigits = 1)
        ellipsefeatures["el_ssumZ" + suffix] = round(ellipsefeatures["el_ssumZ" + suffix], ndigits = 1)
    ...
    if str(membership) == "None":
        ...
        for i in range(iplus1 - n, iplus1):
            ...
            ellipsefeatures[i0].update({"el_ssum" : squaresums[i0], 
    ...
    # Fit ellipse to NPC subcomplexes
        ...
        def ringmem(name):
            ...
            for i in range(iplus1 - n, iplus1):
                ...
                elPart[i0].update({"el_ssum" : squaresums[i0], 
                                        "el_ssumXY" : squaresumsXY[i0],
                                        "el_ssumZ" : squaresumsZ[i0]})
       ...
       for i in range(membership[0], membership[-1]+1):
           ...
           ellipseAll[i0].update({"el_ssum" : squaresums[i0], 
                                        "el_ssumXY" : squaresumsXY[i0],
                                        "el_ssumZ" : squaresumsZ[i0]})
...
def fitEllipse3D(NPC, z = None, ring = None):
    ...
    NPC_centered, V, Ce = fitPlane(NPC[:,:3]) # find plane that minimises distance to points
    ...
    x0 = (2*C*D - B*E)/(B**2 - 4*A*C) # centre of ellipse fitted to NPC_xy. Not necessarily 0, 0
    ...
    keyList = ["el_minor", "el_major", "el_q", "el_rot", 
               "minv", "majv", "Ce", "normal2", "normal1", "normal0"]
    valueList = [minor, major, q, rho, 
                 minv, majv, Ce, normal2, normal1, normal0]
...
def Circles(NPCscoords, membership = None):
        ...
        if str(membership) == "None":
            ...
            i0 = 0
            for i in range(iplus1 - n, iplus1): 
                circlefeatures[i0] = fitCircle3D(NPCscoords[NPCscoords[:,4]==i])  #TODO: 4 
                i0 += 1
...
def fitCircle3D(NPC):     
    ...
    xc, yc, r, residual = cf.least_squares_circle(NPC_xy[:,:2]) #xcentre, ycentre, radius
    
    sqsum = zsqsum + residual
        
    if rotate:
        C = C + rodrigues_rot(np.array([xc, yc, 0]), basisZ, normal2)# Transform circle center back to 3D coords

    C = C.flatten()

    return [r, sqsum, residual, zsqsum, C, normal2, normal1, normal0] #[xc, yc, np.round(r, 2), residual] # TODO: change back
...
def fitCircle(NPC):     
    ...
    xc, yc, r, residual = cf.least_squares_circle(NPC) #xcentre, ycentre, radius #TODO: cf only works in 2D
...
def meanfeaturesE(NPCs, var, ellipse_allrings, el_name=False):
    ...
    for i in range(n):
        ...
        featuresElAll[i] = np.array([np.mean([ellipse_NPC_n[j][i] for j in range_n_rings_in_i]) for i in el_name])                  
        centreElAll[i] = np.mean([ellipse_NPC_n[i]["Ce"] for i in range_n_rings_in_i], axis = 0)   
        tiltElAll[i] = np.mean([ellipse_NPC_n[i]["normal2"] for i in range_n_rings_in_i], axis = 0)
...
def angles(entriestiltAll):
    ...
    for i in range(len(entriestiltAll)):
        ...
        if tiltangles[i] >= np.pi/2: tiltangles[i] = np.pi - tiltangles[i]
